...
import { FaRulerCombined, FaCube, FaInfoCircle, FaLayerGroup, FaPercentage } from "react-icons/fa";
...
const ZoneCard = ({ zone, warehouse, allZones }) => {
    ...
    const initializeContainers = useCallback((zoneId) => {
        if (!containers[zoneId]) {
            const zoneData = allZones.find(z => z.id === zoneId);
            setContainers(prev => ({ ...prev, [zoneId]: zoneData?.containers || [] }));
        }
    }, [allZones, containers]);
    ...
    const handleDeleteZone = useCallback(async (zoneId) => {
        try {
            await axios.delete(
                `http://localhost:8081/api/v1/warehouse-manager/warehouses/${zoneId}/zones`,
    ...
    const handleDeleteContainer = useCallback((zoneId, containerId) => {
        setContainers(prev => ({
            ...prev,
    ...
    const fillPercentage = useMemo(() => {
        const totalCapacity = zone.capacity || 0;
        return totalCapacity > 0 ? Math.round(((totalCapacity - freeCapacity) / totalCapacity) * 100) : 0;
    ...
    const ZoneMenu = ({ zoneId, isParent }) => (
        <div className="relative flex items-center gap-2">
            <span className="text-xs font-medium text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full">#{zoneId}</span>
            <button
            ...
            {menuOpen === zoneId && (
                <div className="absolute right-0 top-full mt-1 w-44 bg-white border border-gray-200 rounded-md shadow-lg z-20 p-1">
                    {isParent && (
                        ...
                        >
                            <BiPlus size={14} className="text-blue-500" /> Подзона
                        </button>
                    )}
                    <button
                        className="flex items-center gap-2 w-full px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 rounded transition-colors duration-150"
    ...
    const ShelfCard = ({ shelf }) => {
        ...
        const shelfFillPercentage = useMemo(() => {
            const totalCapacity = shelf.capacity || 0;
            return totalCapacity > 0 ? Math.round(((totalCapacity - shelfFreeCapacity) / totalCapacity) * 100) : 0;
            ...
            <div className="p-3 bg-white rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-200">
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        <button onClick={() => toggleContainers(shelf.id)} className="p-1 hover:bg-gray-100 rounded-full transition-colors duration-150">
                        ...
                        <div>
                            <div className="text-sm font-medium text-gray-800 flex items-center gap-1">
                                ...
                                {shelf.name}
                                <span className="text-xs text-gray-500 ml-1">({(containers[shelf.id] || shelf.containers || []).length} конт.)</span>
                            </div>
                            <div className="text-xs text-gray-600 mt-1 flex flex-wrap gap-2">
                                <span className="flex items-center gap-1">
                                    <FaRulerCombined size={10} /> {shelfDimensions}
                                </span>
                                <span className="flex items-center gap-1">
                                    <FaCube size={10} /> {shelf.capacity || '-'} м³
                                </span>
                                <span className="flex items-center gap-1">
                                    <FaInfoCircle size={10} className={shelfFreeCapacity > 0 ? 'text-green-500' : 'text-red-500'} />
                                    {shelfFreeCapacity} м³
                                </span>
                                <span className="flex items-center gap-1">
                ...
                {openContainers[shelf.id] && (
                    <div className="mt-2 pl-6">
                        {containers[shelf.id]?.length > 0 || shelf.containers?.length > 0 ? (
                            ...
                            <div className="text-xs text-gray-400 text-center py-1 bg-gray-50 rounded">Нет контейнеров</div>
                        )}
                    </div>
                )}
            </div>
    ...
    return (
        <div className="mb-3 p-4 bg-white rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-all duration-200">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                    <button onClick={toggleShelves} className="p-1 hover:bg-gray-100 rounded-full transition-colors duration-150">
                        <HiArrowSmDown className={`w-6 h-6 text-gray-500 ${openShelves ? 'rotate-180' : ''}`} />
                    </button>
                    <div>
                        ...
                        <div className="text-xs text-gray-600 mt-1 flex flex-wrap gap-2">
                            <span className="flex items-center gap-1">
                                <FaRulerCombined size={12} /> {dimensions}
                            </span>
                            <span className="flex items-center gap-1">
                                <FaCube size={12} /> {zone.capacity || '-'} м³
                            </span>
                            <span className="flex items-center gap-1">
                                <FaInfoCircle size={12} className={freeCapacity > 0 ? 'text-green-500' : 'text-red-500'} />
                                {freeCapacity} м³
                            </span>
                            <span className="flex items-center gap-1">
                                <FaPercentage size={12} className="text-purple-500" /> {fillPercentage}%
                            </span>
                        </div>
                    </div>
                </div>
            ...
            {openShelves && (
                <div className="mt-3 pl-6">
                    {shelves.length > 0 ? (
                        ...
                        <div className="text-xs text-gray-400 text-center py-2 bg-gray-50 rounded">Нет подзон</div>
                    )}
                </div>
            )}
