...
const Warehouse3DViewer = ({ warehouseData, warehouseId, onUpdateWarehouse }) => {
  ...
  const createTextSprite = (text) => {
    ...
    sprite.scale.set(canvas.width / (window.innerWidth <= 640 ? 40 : 30), canvas.height / (window.innerWidth <= 640 ? 40 : 30), 1);
    return sprite;
  };

  const createConnectionLine = (startPos, endPos) => {
    const material = new THREE.LineBasicMaterial({ color: 0x000000, linewidth: window.innerWidth <= 640 ? 1 : 2 });
  ...
  const createZone = (zone, parentGroup, level, parentPosition = { x: 0, y: 0, z: 0 }) => {
    ...
    const edgesMaterial = new THREE.LineBasicMaterial({ color: 0x000000, linewidth: window.innerWidth <= 640 ? 1 : 2 });
  ...
  useEffect(() => {
    ...
    let rootOffsetX = 0;
    const rootPadding = window.innerWidth <= 640 ? 4 : 6;
      ...
      if (showLabels) {
        warehouseGroup.traverse(child => {
          if (child.isSprite && child.userData.level !== undefined) {
            ...
            child.visible = distance > (window.innerWidth <= 640 ? 3 : 5);
          }
        });
      } else {
        warehouseGroup.traverse(child => {
  ...
  return (
    <div className="relative w-full h-[400px] sm:h-[600px] md:h-[800px] border border-gray-300 rounded-xl shadow-lg overflow-hidden bg-gray-50">
      <div ref={mountRef} className="w-full h-full" />

      <div className="absolute top-1 sm:top-2 left-1 sm:left-2 w-36 sm:w-48 bg-white/95 p-2 sm:p-3 rounded-md shadow-lg pointer-events-auto border border-gray-100">
        <h3 className="text-sm sm:text-lg font-semibold text-gray-800 mb-1 sm:mb-2 flex items-center gap-1">
          <FaCube className="text-blue-600 w-3 sm:w-4 h-3 sm:h-4" /> Управление
        </h3>
        <div className="space-y-1 sm:space-y-2">
          <button
            onClick={toggleWireframe}
            className="w-full flex items-center gap-1 px-1 sm:px-2 py-0.5 sm:py-1 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition-all duration-150 text-[10px] sm:text-xs font-medium"
          >
            {isWireframe ? (
              <>
                <FaEye className="w-2 sm:w-3 h-2 sm:h-3" /> Сплошной
              </>
            ) : (
              <>
                <FaEyeSlash className="w-2 sm:w-3 h-2 sm:h-3" /> Каркас
              </>
            )}
          </button>
          <button
          ...
          >
            {showGrid ? (
              <>
                <FaEyeSlash className="w-2 sm:w-3 h-2 sm:h-3" /> Скрыть сетку
              </>
            ) : (
              <>
                <FaEye className="w-2 sm:w-3 h-2 sm:h-3" /> Показать сетку
              </>
            )}
          </button>
          <button
          ...
          >
            {showSubZones ? (
              <>
                <FaEyeSlash className="w-2 sm:w-3 h-2 sm:h-3" /> Скрыть подзоны
              </>
            ) : (
              <>
                <FaEye className="w-2 sm:w-3 h-2 sm:h-3" /> Показать подзоны
              </>
            )}
          </button>
          <button
          ...
          >
            {showLabels ? (
              <>
                <FaEyeSlash className="w-2 sm:w-3 h-2 sm:h-3" /> Скрыть метки
              </>
            ) : (
              <>
                <FaEye className="w-2 sm:w-3 h-2 sm:h-3" /> Показать метки
              </>
            )}
          </button>
          <button
            ...
            className="w-full flex items-center gap-1 px-1 sm:px-2 py-0.5 sm:py-1 bg-gray-600 text-white rounded-md hover:bg-gray-700 transition-all duration-150 text-[10px] sm:text-xs font-medium"
          >
            <FaSyncAlt className="w-2 sm:w-3 h-2 sm:h-3" /> Сброс камеры
          </button>
          <button
            onClick={toggleEditMode}
            className="w-full flex items-center gap-1 px-1 sm:px-2 py-0.5 sm:py-1 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all duration-150 text-[10px] sm:text-xs font-medium"
          >
      ...
      {selectedObject && (
        <div className="absolute top-2 sm:top-4 right-2 sm:right-4 w-64 sm:w-80 bg-white/95 p-3 sm:p-5 rounded-lg shadow-xl pointer-events-auto border border-gray-100">
          <h3 className="text-base sm:text-xl font-bold text-gray-800 mb-2 sm:mb-4 flex items-center gap-1 sm:gap-2">
            {selectedObject.type === 'zone' ? (
              <>
                <FaCube className="text-gray-600 w-3 sm:w-4 h-3 sm:h-4" /> Информация о зоне
              </>
            ) : (
              <>
                <FaCube className="text-gray-600 w-3 sm:w-4 h-3 sm:h-4" /> Информация о контейнере
              </>
            )}
          </h3>
