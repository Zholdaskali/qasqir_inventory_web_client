...
const DocumentGenerator = () => {
  ...
  useEffect(() => {
    const fetchSuppliers = async () => {
      try {
        const response = await axios.get(API_GET_SUPPLIERS, {
          headers: { "Auth-token": authToken },
        });
        setSuppliers(response.data.body);
      } catch (error) {
        toast.error(error.response?.data?.message || "Ошибка загрузки поставщиков");
      }
    };
    fetchSuppliers();
  }, [authToken]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const documentResponse = await axios.post(API_ADD_DOCUMENT, documentPayload, {
        headers: { "Auth-token": authToken },
      });
