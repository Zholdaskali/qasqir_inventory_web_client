...
import { API_GET_INVENTORY_CHECKS_COMPLETED } from "../../../api/API";
...
const InCompleteInventoryPage = () => {
  ...
  useEffect(() => {
    const fetchCompletedInventories = async () => {
      try {
        setLoading(true);
        const response = await axios.get(API_GET_INVENTORY_CHECKS_COMPLETED, {
    ...
    <div className="w-full h-full px-4 py-4 md:px-6 md:py-6 lg:px-8 lg:py-8 rounded-xl overflow-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-6 mt-4">
          {completedInventories.slice().reverse().map((inventory) => {
            const results = inventory.result || [];
